...
- **2026-06-11 (James, build #3, first real session):** **Audible bell when the rest period ends.** Currently rest-end is haptic-only (bell sound exists but is only wired to the EMOM timer). Reuse the bell.wav + expo-audio player in RestTimer's onComplete. Note: while the screen is locked, sound won't fire with JS paused — full solution ties into the lock-screen notification item above (expo-notifications can play sound from a scheduled notification).
